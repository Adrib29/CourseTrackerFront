            ...
            <div class="row">
                <div class="col-md-8" >
                    <GoogleMap 
                        ...
                        <Polyline v-if="coordAppercu.length > 0" :options="{ path: CoordToPosition(coordAppercu),...AppercuePath }" /> 
                ...
                <div class="col-md-4 border shadow p-3 mb-5">
                    ...
                    <form @submit.prevent="addEtape" class="row g-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input v-model="endDate" type="datetime-local" class="form-control" :class="{ 'is-invalid': !dateIsValid }" required>
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control" ref="fileInput" type="file" accept=".gpx" maxlength="200000" :class="{ 'is-invalid': !etapeIsValid || !fichierIsValid}" required @change="handleFileChange">
                                <span class="invalid-feedback" v-if="!etapeIsValid && intputFileIsValid"> Aucune étape trouvée </span>
                                <span class="invalid-feedback" v-if="!fichierIsValid && intputFileIsValid"> Le fichier est trop lourd (200Mo max) </span>
                        ...
                        <div class="mb-3">
                            <button type="submit" class="btn btn-primary mb-3" background-color="#1e3d59" :disabled="!formIsValid" >Ajouter</button>
                        </div>
                    </form>
                </div>
            </div>
...
import { CoordToPosition, CalculDistance, etapeIsOnParcours, traceEtapeOnParcours } from '../utils/Coordonnes';
...
const etapeIsValid = computed(() => { return (coordAppercu.value.length > 2) });
...
const coordAppercu = ref<Array<CoordonnesEtapeModel>>([]);
...
async function handleFileChange(event: Event) {
  ...
  if (target !== null && target.files !== null) {
    ...
    if(target.files.length > 0 && target.files[0].size < 200000 ){
        ...
        coordAppercu.value = coordEnCommun.value;
        }else{
        coordAppercu.value.splice(0, coordAppercu.value.length);
        }
        }else{
...
const formData = new FormData();
    ...
    coord.value = await processGPXFile(fichierGpx.value, 1);
    try{
    if(coord){
