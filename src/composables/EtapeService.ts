...
export function useEtapeService() {
  return {
    ...
    async getEtape(parcoursId: number, etapeId: number): Promise<EtapeModel> {
      console.log(etapeId);
      const response = await axios.get<EtapeModel>(`http://localhost:8081/parcours/${parcoursId}/etape/${etapeId}`);
      return response.data;
    },
