      ...
      <div class="col-md-6 col-lg-4" v-for="(parcours, index) in parcoursList" :key="index">
        <div class="card text-center mb-3 shadow p-3 mb-5 bg-body-tertiary rounded" style="width: 18rem;">
          <div class="card-header d-flex justify-content-between">
            ...
            <DeleteParcours :parcours-model="parcours" class-button="btn-close btn btn-large btn-danger"
              @deleted="updateParcours"> </DeleteParcours>
          </div>
          <div class="progress">
            <div class="progress-bar overflow-visible" role="progressbar" style="min-width: 2em;"
              :style="{ width: distanceRealise.at(index) + '%' }" :aria-valuenow="distanceRealise.at(index)?.toString()"
              aria-valuemin="0" aria-valuemax="100">{{ distanceRealise.at(index) }}%</div>
          </div>
          <div class="card-body ">
            <RouterLink class="btn btn-primary" :to="{ name: 'parcoursDetail', params: { parcoursId: parcours.id } }">
              Détails </RouterLink>
          </div>
        </div>
      </div>
