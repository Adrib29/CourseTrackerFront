<template>
  ...
  >
    ...
    <Polyline :options="flightPathdone" />
     <Marker :options="Options" @dragend="onMarkerDragEnd"
     />
    <Marker v-for="(marker, index) in markers" :key="index" :options={position:marker.position,...markerOptions} />
  </GoogleMap>
</template>

<script>
import { defineComponent } from 'vue'
import { GoogleMap, Marker, Polyline, CustomMarker, MapCluster } from 'vue3-google-map'
import { ref } from 'vue';
export default ({
  ...
  setup() {
    ...
    let long = 0.0;
    let lat = 0.0;
    const center = { lat: 49.4975, lng: 0.20198 }
    let markerPosition = { lat: 49.4975, lng: 0.20198 }
    const markerOptions = { label: "L", title: "LADY LIBERTY", primaryColor: "#0000FF", draggable: true, clickable: true};
    const Options = {position:center, label: "S", title: "LADY LIBERTY", primaryColor: "#0000FF", draggable: true, clickable: true};    
    ...
    function addMarker(latitude,longitude) {
      ...
      markers.value.push({
        position: { lat: 49.50429, lng: 0.13121 },
      });
    }
  ...
  methods: {
   Test(event){
      if (this.markerLoaded) {
        ...
        this.$refs.marker.setPosition(newPosition);
   ...
   onMarkerDragEnd(event) {
      ...
      this.addMarker(closestPoint.lat,closestPoint.lng);
